...
const SingleMovieDetailsNormalCarousel = (props) => {
  ...
  useEffect(()=>{
    if(display){
      const wordList = movie.url.replace("https://www.pexels.com/video/", "").match(/[a-zA-Z]+/g)
      const tempRandomWord = wordList[Math.floor(Math.random() * wordList.length)]
  ...
  useEffect(() => {
    if (sessionStorage.getItem("movieNMDetails")) {
        const retrievedData = JSON.parse(sessionStorage.getItem("movieNMDetails"));
        setDisplay(true);
      ...
      {display ? (
        ...
        <div className={styles.container}>
          <div className={styles.singleMovie}>
            ...
            <MoreSimilarMovies
              similarMovies={similarContent}
              />
            <button onClick={props.movieDetailsToggle} className={styles.closeButton}>X</button>
          </div>
        </div>
      </div>
      ) : (
